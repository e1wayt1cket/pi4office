...
**PowerPoint tools** — slide-level interaction:

| Tool | What it does |
|---|---|
| `get_presentation_overview` | Slide index, title, and layout summary |
| `read_slide` | Read slide shapes and text content by index |
| `search_presentation` | Search across all slides for matching text |
| `add_slide_notes` | Add or replace text on a specific slide shape |
| `create_slide` | Add a new slide with configurable layout, title, and content |
...
4. Open Excel, Word, or PowerPoint → **Home** → **Add-ins** → **Pi for Office**
5. Connect a provider (API key or OAuth), or configure a custom gateway in `/settings`
6. Start chatting — try `What sheets do I have?`, `Outline this document`, or `Create a slide about Q3 results`
...
2. Add it to Excel, Word, or PowerPoint — see [**install guide**](docs/install.md) for step-by-step instructions (macOS + Windows)
...
5. Start chatting — try `What sheets do I have?`, `Outline this document`, or `Create a slide about Q3 results`
